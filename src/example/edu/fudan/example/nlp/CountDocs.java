...
class CountDocs {
	public  CountDocs(String time,String title,String text){
		this.time = time;
		this.title = title;
		this.text = text;
	}
	...
	public static void main(String[] args) throws SQLException, LoadModelException, IOException {
		...
		extractout = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("data/count20140828.txt")));
		...
		for (int i = 1; i <= 1373928; i++) {
			...
			try {
				while (rs_content.next() && rs_meta.next()) {
					...
					//环境污染+环境保护+环境质量+空气质量+空气污染+大气污染+大气质量
					if ((content.contains("环境污染") 
							|| content.contains("环境保护")
							|| content.contains("环境质量")
							|| content.contains("空气质量")
							|| content.contains("空气污染")
							|| content.contains("大气污染")
							|| content.contains("大气质量")
							) && !text.contains("气候变化") && !text.contains("全球变暖")){
						BufferedWriter bw;
						try {
							System.out.println(dates);
							extractout.write(dates+"_1\n");
